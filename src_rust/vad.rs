...
// ── find_vwp_header_offset ──

#[pyfunction]
pub fn find_vwp_header_offset(data: &[u8]) -> PyResult<Option<usize>> {
    if data.len() < 2 {
        return Ok(None);
    }
...
// ── VadRecord+parse_vwp_tabular_data ──
...
#[pyfunction]
pub fn parse_vwp_tabular_data<'py>(
    py: Python<'py>,
    data: &[u8],
...
#[pyfunction]
pub fn vec2comp(wdir: f64, wspd: f64) -> PyResult<(f64, f64)> {
...
#[pyfunction]
pub fn comp2vec(u: f64, v: f64) -> PyResult<(f64, f64)> {
...
// ── _clip_profile_internal ──

pub(crate) fn _clip_profile_internal(
    wind_dir: &[f64],
    wind_spd: &[f64],
    altitude: &[f64],
    max_hght: f64,
...
pub(crate) fn _interp_linear(x: &[f64], y: &[f64], xi: f64) -> f64 {
