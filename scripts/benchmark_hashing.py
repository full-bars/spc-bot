...
def main():
    ...
    print(f"Python (SHA256 baseline): {python_time:.4f}s ({python_time/iterations:.6f}s per hash)")
    ...
    if RUST_AVAILABLE:
        ...
        print(f"Rust (XXH3 optimization): {rust_time:.4f}s ({rust_time/iterations:.6f}s per hash)")
