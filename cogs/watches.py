...
async def fetch_active_watches_nws() -> Dict[str, dict]:
    ...
    for feature in data.get("features", []):
        ...
        for vtec in vtec_list:
            m = re.search(r"/[^.]+\.[^.]+\.[^.]+\.(SV|TO)\.A\.(\d{4})\.", vtec, re.IGNORECASE)
            if not m:
                continue
            ...
            if expires_str:
                try:
                    ...
                    pass
            logger.info(f"[WATCH] NWS API: #{watch_num} ({wtype}) expires {expires_dt}")
...
async def fetch_latest_watch_numbers() -> List[Tuple[str, str]]:
    ...
    results = []
    seen = set()
    for m in re.finditer(r'href="ww(\d+)\.html"[^>]*alt="([^"]*)"', html, re.IGNORECASE):
...
async def fetch_watch_details(watch_number: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    """
            ...
            for line in lines:
                ...
                if in_threats:
                    ...
                    if line and not line.startswith('*'):
                        if threats and not re.search(r'possible$|mph$|diameter$', threats[-1], re.IGNORECASE):
                            threats[-1] += ' ' + line
                        else:
                            threats.append(line)
    ...
    if prob_html:
        ...
        if pairs:
            sections = {'Tornado': [], 'Wind': [], 'Hail': [], 'Combined': []}
            ...
            if prob_lines:
                probs = '\n'.join(prob_lines)
            logger.info(f"[WATCH] Parsed {len(pairs)} prob entries for #{watch_number}")
        else:
            logger.warning(f"[WATCH] No prob pairs parsed for #{watch_number}")
...
class WatchPaginatorView(discord.ui.View):
    ...
    def build_embed(self):
        ...
        embed.set_footer(
            text=f"Watch {self.index + 1} of {len(self.watch_data)} · SPC Watch Monitor"
        )
        if cache_path:
...
async def _execute_watches(interaction: discord.Interaction):
    ...
    for watch_num, nws_info in nws_watches.items():
        ...
        watch_data.append((watch_num, nws_info, image_url, text_summary, probs, cache_path))
    ...
    msg = await interaction.followup.send(embed=embed, files=files, view=view)
    view.message = msg
...
class WatchesCog(commands.Cog):
    ...
    async def auto_post_watches(self):
        ...
        try:
            ...
            for watch_num, info in list(active_watches.items()):
                ...
                logger.info(f"[WATCH] Watch #{watch_num} {reason} — posting cancellation")
                ...
                try:
                    ...
                    logger.info(f"[WATCH] Posted cancellation for #{watch_num}")
                except discord.HTTPException as e:
                    logger.error(f"[WATCH] Failed to send cancellation for #{watch_num}: {e}")
            ...
            for watch_num, nws_info in nws_watches.items():
                ...
                logger.info(f"[WATCH] New watch detected: #{watch_num} ({wtype})")
                image_url, text_summary, probs = await fetch_watch_details(watch_num)
                cache_path = None
                if image_url:
                    cache_path, _, _ = await download_single_image(
                ...
                try:
                    files = [discord.File(cache_path, filename=f"watch_{watch_num}.gif")] if cache_path else []
                    ...
                    logger.error(f"[WATCH] Discord send failed for #{watch_num}: {e}")

        except Exception as e:
            logger.error(f"[WATCH] Unexpected error in auto_post_watches: {e}", exc_info=True)

    @auto_post_watches.after_loop
    async def after_watches_loop(self):
        if self.auto_post_watches.is_being_cancelled():
            return
        exc = self.auto_post_watches.get_task().exception() if self.auto_post_watches.get_task() else None
        if exc:
            logger.error(
    ...
    @discord.app_commands.command(name="watches", description="Show all currently active SPC watches")
    async def watches_slash(self, interaction: discord.Interaction):
        await _execute_watches(interaction)

    @discord.app_commands.command(name="ww", description="Show all currently active SPC watches")
