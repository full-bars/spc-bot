...
def _plot_skewt_fast(clean_data: dict, output_path: str, dark_mode: bool):
    """Ultra-fast placeholder Skew-T (<0.2 s after first import in this worker).
    Plots T, Td, and wind barbs on a bare Skew-T axis — no CAPE shading,
    no parcel paths, no hodograph inset.  The message is later upgraded to
    the full SounderPy plot via message.edit."""
    try:
        from metpy.plots import SkewT
        from metpy.units import units
    except ImportError:
        return _plot_sync(clean_data, output_path, dark_mode)

    import matplotlib.pyplot as plt
    import numpy as np

    p = np.array([v for v in clean_data["p"].m if not np.isnan(v)])
    t_arr = np.array([v.m for v in clean_data["T"] if not np.isnan(v.m)])
    td_arr = np.array([v.m for v in clean_data["Td"] if not np.isnan(v.m)])
    u_arr = np.array([v.m for v in clean_data["u"] if not np.isnan(v.m)])
    v_arr = np.array([v.m for v in clean_data["v"] if not np.isnan(v.m)])
    ...
    skew.plot(p * units.hPa, t_arr * units.degC, "r", linewidth=2, label="T")
    skew.plot(p * units.hPa, td_arr * units.degC, "g", linewidth=2, label="Td")

    barb_p = p[::2]
    barb_u = u_arr[::2]
    barb_v = v_arr[::2]
    if len(barb_p) > 1:
        skew.plot_barbs(barb_p * units.hPa, barb_u * units.knots, barb_v * units.knots)

    skew.ax.legend(loc="lower right", fontsize=9)
    skew.ax.set_title("Loading full plot…", fontsize=10, color="gray", style="italic")

    fig.savefig(output_path, bbox_inches="tight", dpi=72)
    plt.close("all")
