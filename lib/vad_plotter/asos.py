...
async def get_asos_surface_wind(
    radar_lat: float,
    radar_lon: float,
    vwp_time: Optional[datetime] = None,
    radius_km: float = 150,
    ...
    if not best_by_station:
        logger.debug("[ASOS] No stations found with valid wind data")
        return None

    candidates = sorted(best_by_station.values(), key=lambda x: x["dist"])
    best = candidates[0]
    obs_dt = datetime.fromtimestamp(best["obs_ts"], tz=timezone.utc).strftime(
        "%H%M"
    )
    logger.debug(
        f"ASOS surface wind: {best['sid']} {best['wdir']:03d}/{best['wspd']:02d}kt "
