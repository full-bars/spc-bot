...
async def summarize_sounding(raw_text: str) -> str | None:
    prompt = (
        ...
        "PARAMETER SCALES FOR REFERENCE:\n"
        "• CAPE (J/kg): <100 = weak instability, 100-500 = moderate, 500-1500 = strong, >1500 = extreme\n"
        "• SRH (m²/s²): <100 = poor shear, 100-200 = marginal, 200-400 = significant, >400 = extreme\n"
        "• DCAPE (J/kg): >400 = strong downdraft potential\n"
        "• Lapse Rate (K/km): >8 = steep (unstable), 6-8 = moderately steep, <6 = stable\n\n"
        "CRITICAL INSTRUCTIONS FOR INCOMPLETE PROFILES (ACARS, hodographs):\n"
        "• Evaluate ALL available thermodynamic parameters: if SB 0-3km CAPE is high, "
        "low-level instability exists even if full-column CAPE is N/A\n"
        "• Examine lapse rates, CIN, and low-level indices (LCL, SRH 0-1km) as primary instability signals\n"
        "• If multiple low-level indices show strong values (e.g., high 0-3km CAPE + low LCL + high SRH), "
        "the environment supports severe weather despite profile limitations\n"
        "• Never discount a sounding based solely on N/A values; assess the complete parameter set\n\n"
        f"DATA:\n{raw_text}"
    )
    return await call_gemini(prompt)
...
async def summarize_sounding_enhanced(
    ...
    prompt = (
        ...
        "PARAMETER SCALES FOR REFERENCE:\n"
        "• CAPE (J/kg): <100 = weak, 100-500 = moderate, 500-1500 = strong, >1500 = extreme\n"
        "• SRH (m²/s²): <100 = poor, 100-200 = marginal, 200-400 = significant, >400 = extreme\n"
        "• Lapse Rate (K/km): >8 = steep, 6-8 = moderate, <6 = stable\n"
        "• DCAPE (J/kg): >400 = strong downdraft potential\n\n"
