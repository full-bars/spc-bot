...
async def test_finalize_mission_flow():
    ...
    with patch("cogs.recorder.get_executor"), patch("cogs.recorder.os.makedirs"):
        cog = RecorderCog(bot)
    ...
    with patch("asyncio.get_running_loop") as mock_loop:
        ...
        mock_run.side_effect = [
            ...
            True,
            True,  # _render_frame_worker (twice)
            None,  # _make_gif
            150.0,  # _calc_srh
            None,  # _cleanup
        ]
        ...
        # Mock dependencies
        with patch(
            "utils.events_db.update_event_environment", new_callable=AsyncMock
        ) as mock_update_db, patch.object(
            cog, "_post_forensic_summary", new_callable=AsyncMock
        ) as mock_post:
