...
async def test_fetches_from_nws_on_memory_miss(cog):
    ...
    with patch("cogs.sounding.get_watch_centroid_cache", AsyncMock(return_value=None)), patch(
        "cogs.sounding.get_watch_area_centroid", AsyncMock(return_value=(35.5, -97.5))
...
async def test_returns_none_when_no_zones(cog):
    ...
    with patch("cogs.sounding.get_watch_centroid_cache", AsyncMock(return_value=None)), patch(
        "cogs.sounding.get_watch_area_centroid"
    ) as nws_get:
        result = await cog._resolve_watch_centroid("0042", info)
...
async def test_does_not_cache_none_from_api(cog):
    ...
    with patch("cogs.sounding.get_watch_centroid_cache", AsyncMock(return_value=None)), patch(
        "cogs.sounding.get_watch_area_centroid", AsyncMock(return_value=None)
    ), patch("cogs.sounding.set_watch_centroid_cache", AsyncMock()):
        result = await cog._resolve_watch_centroid("0042", info)
