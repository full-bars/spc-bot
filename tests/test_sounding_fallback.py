...
async def test_fetch_sounding_falls_back_to_gsl_on_iem_failure():
    ...
    # Mock IEM to fail
    with patch("cogs.sounding_utils.fetch_iem_sounding", AsyncMock(return_value=None)), patch(
        "cogs.sounding_utils.fetch_gsl_sounding", AsyncMock()
    ) as mock_gsl:
...
async def test_fetch_sounding_skips_iem_when_circuit_open():
    ...
    with patch("utils.http.circuit_breaker.is_open", return_value=True), patch(
        "cogs.sounding_utils.fetch_iem_sounding", AsyncMock()
    ) as mock_iem, patch("cogs.sounding_utils.fetch_gsl_sounding", AsyncMock()) as mock_gsl:
...
async def test_fetch_sounding_standard_hour_wyoming_first():
    station_id = "KILX"
    # 12z sounding
    with patch("sounderpy.get_obs_data") as mock_wyo, patch(
        "cogs.sounding_utils.validate_sounding_data", return_value=True
    ):
