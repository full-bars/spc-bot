...
def _good_profile(n=12):
    ...
    return [
        _good_level(
            float(p), tmpc=20 - i, dwpc=15 - i, drct=200 + i * 5, sknt=10 + i * 3, hght=i * 1000.0
        )
...
class TestSoundingQualityWarning:
    ...
    def test_warns_on_shallow_pressure_span(self):
        ...
        profile = [_good_level(float(p), drct=200 + i, sknt=10) for i, p in enumerate(pressures)]
        out = _iem_to_clean_data(
            profile,
            "KILX",
            "Lincoln",
...
"""
        out = _fsl_to_clean_data(
            sample_fsl, "KILX", "Lincoln", 40.15, -89.33, 180, ["2026", "05", "01", "12:00"]
        )
        assert out is not None
