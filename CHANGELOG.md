...
- **`/channels assign` and `/channels list`**: Replaced `/enablewarnings`, `/disablewarnings`, and `/displaysetup` with a `/channels` command group. `/channels assign <product> <channel>` routes a product type to a channel; omitting the channel stops posting that type entirely — replacing the old multi-select "disable" flow with a single-type-at-a-time model. `/channels list` shows the full routing table. Now covers NHC Tropical Products in addition to TOR/SVR/FFW/SPS warnings.
