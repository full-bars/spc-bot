...
- **`WEATHER_CHAT_CHANNEL_ID`**: New optional `.env` variable for `cogs/fronts.py`'s weather-chat channel, defaulting to `SPC_CHANNEL_ID` if unset — replaces a hardcoded channel ID.
- **`utils/discord_send.py`**: New `safe_send()`/`safe_create_thread()` helpers for background auto-posting cogs. Both catch `discord.Forbidden` specifically and log a clear "missing permissions in #channel" error instead of a generic exception, so a misconfigured channel's permissions are visible in logs instead of silently swallowed. Applied across all background task-loop posts (tropical, mesoscale/MD, outlooks, watches, warnings, scp, ncar, reports, recorder) — interactive slash-command replies were left as-is since those already surface errors to the invoking user.
...
- **Tropical Products Posting to Public Channel While Still Being Tested**: The NHC tropical auto-poster had no channel override configured, so it defaulted straight to the public production channel while the feature was still being validated (and was in fact repeatedly hitting `discord.errors.Forbidden` there — see below). Now routed to the dev channel via the `warning_channel:tropical` state override until the feature is ready for production.
- **Hardcoded Discord Channel IDs**: Removed a hardcoded fallback channel ID from `config.py`'s `tropical_channel_id` and a hardcoded, unused `DEV_CHANNEL_ID` constant from `cogs/tropical.py`, plus a hardcoded `WEATHER_CHAT_CHANNEL_ID` in `cogs/fronts.py` — all now resolve through `.env`-backed config with the same fallback-chain convention as every other channel.
- **Silent Tropical-Poster Permission Failures**: The tropical auto-poster was failing every post attempt against the production channel with `discord.errors.Forbidden: 403 (Missing Permissions)`, logged only as a generic exception buried in routine log volume — nothing distinguished it from a transient network error. `safe_send()` now logs permission failures at ERROR with the channel name/ID called out explicitly.
- **Tropical Dedup-Before-Send**: `post_tropical_product` marked a product as posted *before* attempting to send it, so a failed send (e.g. the permissions issue above) would permanently skip that product — it would never be retried even after the underlying problem was fixed. Dedup now only happens after a confirmed successful send.
